...
6. Добавить документ **ПоступлениеТоваровИУслуг**:
  ...
  * Создать форму документа, в которой:
    ...
    * Реализовать подбор аналогично документу **УстановкаЦен**, передавая в качестве владельца открываемой формы таблицу ТоварыИУслуги. При подборе номенклатуру цена не должна заполняться из регистра т.к. цену покупки заполняет пользователь.
